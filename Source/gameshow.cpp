...
GameShow::GameShow(Level *level, QWidget *parent, bool ai) :
        QWidget(parent), ui(new Ui::GameShow), level(level), ai(ai) {
    ui->setupUi(this);
    ...
    for (int i = 0; i < level->getSize()[0]; i++)
        for (int j = 0; j < level->getSize()[1]; j++)
            points[i * level->getSize()[1] + j] = QPoint((int) ((2 * topLeft.x() + (2 * j + 1) * wid) / 2),
                                                         (int) ((2 * topLeft.y() + (2 * i + 1) * hei) / 2)),
                    flag[i * level->getSize()[1] + j] = false;

    lines = level->getLines();
    correct = new bool[level->getNum()];
    for (int i = 0; i < level->getNum(); i++)
        flag[lines[i].start.x() * level->getSize()[1] + lines[i].start.y()] = flag[lines[i].end.x() *
                                                                                   level->getSize()[1] +
                                                                                   lines[i].end.y()] = true, correct[i] = false;
}
...
void GameShow::paintEvent(QPaintEvent *event) {
    ...
    if(ai){
        for (int i = 0; i < level->getNum(); i++) {
            ...
            QPoint start = points[lines[i].start.x() * level->getSize()[1] + lines[i].start.y()];
            QPoint end = points[lines[i].end.x() * level->getSize()[1] + lines[i].end.y()];
            QPoint final = (start + end)/2;
            double len =  sqrt(pow(lines[i].start.x() - lines[i].end.x(), 2) + pow(lines[i].start.y() - lines[i].end.y(), 2));
            painter.drawText(final, QString::number(len, 'f', 1));
        }
    }
}
...
void GameShow::stepBack() {
    if (!step.empty()) {
        if (step.back()) {
            temp = green_lines.back().start.x() * level->getSize()[1] + green_lines.back().start.y();
            correct[inLine(green_lines.back().start.x() * level->getSize()[1] + green_lines.back().start.y(),
                           temp)] = false;
            green_lines.pop_back();
        } else {
            temp = red_lines.back().start.x() * level->getSize()[1] + red_lines.back().start.y();
            red_lines.pop_back();
        }
        step.pop_back();
        if (step.empty())

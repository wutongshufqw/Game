...
GameSelect::GameSelect(QWidget *parent) :
    ...
    connect(ui->difficulty_1, &QPushButton::clicked, dynamic_cast<MainWindow*>(parent->parentWidget()), &MainWindow::press);

    connect(ui->difficulty_2, &QPushButton::clicked, this, [=]() { emit play_game(new GamePlay(1, parent,true,2)); });
    connect(ui->difficulty_2, &QPushButton::clicked, dynamic_cast<MainWindow*>(parent->parentWidget()), &MainWindow::press);

    connect(ui->difficulty_3, &QPushButton::clicked, this, [=]() { emit play_game(new GamePlay(1, parent,true,3)); });
    connect(ui->difficulty_3, &QPushButton::clicked, dynamic_cast<MainWindow*>(parent->parentWidget()), &MainWindow::press);
    ...
    connect(ui->back_button, &QPushButton::clicked, dynamic_cast<MainWindow *>(parent->parentWidget()), &MainWindow::press);
}
